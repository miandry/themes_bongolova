...
const canApply = computed(() => isLoggedIn.value && authRole.value === 'candidate')
...
<template>
  <!-- Sidebar CTA -->
  <div v-if="variant === 'sidebar'" class="mt-6 pt-4 border-t border-gray-100">
    <div v-if="applySuccess" class="w-full py-4 bg-green-50 border border-green-200 text-green-700 rounded-xl font-bold text-center flex items-center justify-center gap-2 text-sm">
  ...
  >
    ...
    <div v-else-if="needsLogin" class="py-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200">
      ...
      <p class="text-gray-500 text-sm mb-4">Créez un compte candidat ou connectez-vous pour envoyer votre candidature.</p>
    ...
    <div v-else-if="wrongRole" class="py-6 px-4 bg-amber-50 border border-amber-200 rounded-xl flex gap-3">
      ...
      <div>
        ...
        <p class="text-amber-700 text-xs mt-1">Seuls les comptes <strong>candidat</strong> peuvent postuler. Déconnectez-vous et reconnectez-vous avec un compte candidat.</p>
      </div>
    </div>
