...
function getPhotoUrl(photoPath: string | null | undefined): string | null {
  ...
  // Si le chemin commence déjà par /sites/... on le garde tel quel
...
<template>
  <section class="py-16 px-4 bg-gradient-to-b from-white to-gray-50 relative overflow-hidden">
    <div
      class="absolute top-0 right-0 w-80 h-80 bg-purple-200/20 rounded-full blur-3xl translate-x-1/3 -translate-y-1/3">
    </div>
    <div
      class="absolute bottom-0 left-0 w-80 h-80 bg-pink-200/20 rounded-full blur-3xl -translate-x-1/3 translate-y-1/3">
    </div>
    <div class="max-w-7xl mx-auto relative z-10">
      <div class="text-center mb-10">
        <div
          class="inline-flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-full mb-3 backdrop-blur-sm border border-purple-200/30">
        ...
        <h2 class="text-3xl md:text-4xl font-black text-gray-900 mb-2">Top <span
            class="bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">Talents</span> de la
          région</h2>
        ...
        <div v-for="candidate in topCandidates" :key="candidate.id"
          ...
          <div class="p-5 text-center">
            <div class="relative inline-block mb-3">
              ...
              <div
                class="w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white text-2xl font-bold shadow-md mx-auto">
                <img v-if="candidate.photo_path" :src="getPhotoUrl(candidate.photo_path)"
                  :alt="`${candidate.first_name} ${candidate.last_name}`" class="w-full h-full object-cover" />
                <span v-else>{{ getInitials(candidate) }}</span>
              </div>
              <div
            ...
            <h3 class="text-base font-bold text-gray-900 group-hover:text-purple-600 transition">{{ candidate.first_name
            ...
            <!-- <div class="flex flex-wrap justify-center gap-1 mt-3 mb-4">
              <span v-for="skill in asList(candidate.skills).slice(0, 3)" :key="skill"
                class="px-2 py-0.5 bg-gray-100 rounded-full text-[9px] text-gray-600">{{ skill }}</span>
            </div> -->
            <a :href="`/profils/${candidate.id}`">
              <button
                class="w-full py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg text-xs font-bold hover:shadow-md transition-all duration-300 flex items-center justify-center gap-1 group/btn">
                Voir le profil
      ...
      <div class="text-center mt-10">
        <a href="/profils">
          <button
            class="group relative px-6 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-full font-bold text-sm hover:shadow-xl transition-all inline-flex items-center gap-2 overflow-hidden">
